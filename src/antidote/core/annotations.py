import builtins
import inspect
from typing import TypeVar, Callable, Hashable, TYPE_CHECKING, Optional, Union
...
class SupportsRMatmul(Protocol):
    def __rmatmul__(self, type_hint) -> Hashable:
        pass
...
class From(FinalImmutable, AntidoteAnnotation):
    ...
    :py:func:`.factory`, :py:class:`.Factory` and :py:func:`.implementation` typically.
    ...
    .. doctest:: core_annotations_from
        ...
        'localhost:6789'
        >>> DifferentDatabase = Annotated[Database,
        ...                               From(build_db.with_kargs(host='different'))]
        >>> # Annotations are also supported in world.get()
        ... world.get[Database](DifferentDatabase).host
        'different'
        >>> # But aren't necessary. However, it's more convenient if you use them
        ... # already as type hints.
        ... world.get[Database](Database @ build_db.with_kargs(host='different')).host
        'different'
...
class FromArg(FinalImmutable, AntidoteAnnotation):
    ...
    def __init__(self,
        ...
        if callable(__function):
            ...
            raise TypeError(f"Expected a string or a function, not {type(__function)}")
...
def extract_argument_dependency(argument: Argument) -> object:
    ...
    if origin is Union:
        if len(args) == 2:
            if isinstance(None, args[1]):  # type: ignore
                dependency = args[0]

    # Annotation without any AntidoteAnnotation
    if origin is Annotated:
        dependency = argument.type_hint
